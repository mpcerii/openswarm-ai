...
import { openSwarmTheme } from "@opencode-ai/ui/context/marked-theme"
import {
  bundledLanguages,
  createHighlighter,
  getTokenStyleObject,
...
const parser = createMarkdownParser(async (code, language) => {
  ...
  return instance.codeToHtml(code, { lang: name as BundledLanguage, theme: "openSwarm", tabindex: false })
})
...
async function highlight(request: Extract<MarkdownWorkerRequest, { type: "highlight" }>) {
  try {
    ...
    if (request.complete) {
      const result = instance.codeToTokens(request.text, { lang: language as BundledLanguage, theme: "openSwarm" })
    ...
    const stream = reset
      ? {
          language,
          source: "",
          tokenizer: new ShikiStreamTokenizer({ highlighter: instance, lang: language, theme: "openSwarm" }),
        }
      : previous
...
function getHighlighter() {
  return (highlighter ??= createHighlighter({ themes: [openSwarmTheme], langs: [] }))
}
