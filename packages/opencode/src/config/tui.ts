...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const dirs = unique(directories).filter((dir) => dir.endsWith(".opencode") || dir.endsWith(".openswarm") || dir === Flag.OPENCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".opencode") && !dir.endsWith(".openswarm") && dir !== Flag.OPENCODE_CONFIG_DIR) continue
