...
// etc.).
//
// The visual language mirrors the openSwarm app: the design tokens are a curated
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `openSwarm is now connected to ${escapeHtml(provider)}.` : "openSwarm is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `openSwarm couldn't finish connecting to ${escapeHtml(provider)}.`
        : "openSwarm couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from openSwarm.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · openSwarm</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("openSwarm couldn't finish connecting to "+PROVIDER+"."):"openSwarm couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from openSwarm."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("openSwarm is now connected to "+PROVIDER+"."):"openSwarm is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
// openSwarm wordmark — same path geometry as packages/ui/src/components/logo.tsx (Logo).
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 234 42" fill="none" aria-label="openSwarm" role="img">
