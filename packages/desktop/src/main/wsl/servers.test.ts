...
test("rejects an update that did not install the desktop version", () => {
  ...
  expect(() => expectOpencodeVersion("1.14.35", "1.16.2")).toThrow(
    "openSwarm update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating openSwarm", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("ignores stale background openSwarm checks after removing a WSL server", async () => {
  persistedServers = []
...
test("ignores stale startup openSwarm checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking openSwarm", async () => {
  persistedServers = []
...
test("does not check openSwarm in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const opencode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
